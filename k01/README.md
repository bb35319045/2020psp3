...
## ソースコードの説明
67: リニアサーチにおけるposを pos_Linear　で定義
68: リニアサーチにおける結果を　result_Linear で定義し、-1で初期化しておく
70:pos_Linerがsize-1になるまで繰り返すfor文
72~75:もし、key == arrayCity[pos_Linear].id
ならば　result_Linearにpos_Linearを代入し、breakでループから出る。
80:　result_Linearの値を返す

85: バイナリーサーチにおけるposを pos_Binary　で定義
86: バイナリーサーチにおける結果　result_Binary で定義し、-1で初期化しておく
88:条件式
90: pos_Binaryを更新
91~93:もし、key == arrayCity[pos_Binary].id
ならば　result_Binaryrにpos_Binaryを代入し、breakでループから出る。
94~99: keyの値の大小によってleft,rightの値を変える
101: result_Binaryの値を返す
